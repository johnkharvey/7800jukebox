;################################################################
; "Jukebox" for the Atari 7800
; by John K. Harvey
;################################################################

	include "hdr/maria.h"
	include "hdr/pokey_0450.h"
...
HighlightIndex			ds	1	;	$42
;=======================================
; These items are in src/pokeymusic.asm
;======================================-
; POKEY-specific for src/pokeymusic.asm
PokeyRAM			ds	16	;	$43,$44,$45,$46
						;	$47,$48,$49,$4A
						;	$4B,$4C,$4D,$4E
						;	$4F,$50,$51,$52

; This is a flag so we can shut off music with the difficulty switches
InitialDifficultySwitches	ds	1	;	$53

; These next 2 variables are used by src/pokeymusic.asm but can be used
; in other pieces of code; they are only used in subroutines for
; temporary byte and word storage.
Temp				ds	1	;	$54
IndirPtr			ds	2	;	$55-$56
;======================================-
...
RamCleanupLoop3
	...
	; That probably took awhile, so maybe we do one of these for good measure
	;===================
	JSR	WaitVBLANK
	;===================
	...
	;=============================================================
	; This us used to shut off POKEY sounds if we don't want them
	;=============================================================
	LDA	SWCHB
	AND	#%11000000
	STA	InitialDifficultySwitches
	;=============================================================
...
Initialization
	LDA	#$C3 ; green
	STA	BACKGRND
	LDA	#$0 ; Let's turn both Joystick ports back to INPUTS (if we were initializing a SaveKey, do this after it's done)
	STA	SWACNT

	; Set a value one higher (remember, we're zero indexed)
	LDA	#NUMBER_OF_SONGS
	STA	HighlightIndex
...
Copy_CHMAPs_Loop2:
	...
	;==================================================
	; Subroutine - Copy the DL's into the RAM at $1900
	;==================================================
Copy_DLs:
	LDX	#$00
Copy_DLs_Loop:
...
AfterLeft
	...
	INC	HighlightIndex
	LDA	HighlightIndex
	CMP	#NUMBER_OF_SONGS+1
	BEQ	FirstTimeChoosingSong
	CMP	#NUMBER_OF_SONGS
	BNE	HandleMusicDownPressed
FirstTimeChoosingSong
	LDA	#0
	STA	HighlightIndex
HandleMusicDownPressed
	TAX
	; Since we can't JSR (indir), we use a helper function.
	; Requires X as an input
	JSR	callJumpTableSubroutine
...
AfterDown
	...
	LDA	HighlightIndex
	CMP	#$FF
	BNE	HandleMusicUpPressed
	LDA	#NUMBER_OF_SONGS-1
	STA	HighlightIndex
HandleMusicUpPressed
	TAX
	; Since we can't JSR (indir), we use a helper function.
	; Requires X as an input
	JSR	callJumpTableSubroutine
...
PlaylistMSB
	...
	dc.b	#>SeedPokeyForSpelunker3
	dc.b	#>SeedPokeyForSpelunker4
	dc.b	#>SeedPokeyForSpelunker5
	dc.b	#>SeedPokeyForSpelunker6
	dc.b	#>SeedPokeyForSpelunker7
...
DLL_On_Screen	equ	Code_DLL_On_Screen - DLL_PAL + DLLRam
	...
	dc.b	$07, >DL_RAM_Start, <DL_Space	; [80] DL for Song ID
